...
mask = np.where((pha>0) & (pha<160000))[0]      
mask_col = np.where((x_abs<=7) & (x_abs>=-7))[0]
mask_row = np.where((y_abs<=7) & (y_abs>=-7))[0]


#limits=[-1.2,0.0,5.5,-5.5] # gpd35 martina  rot 90
limits=[-7,7,7,-7]
#limits=[-5.5,5.5,0,-1.2]  # gpd35 martina rot 0


#limits=[-5.5,5.5,-0.2,-1.4]  # gpd30 martina rot 0
#limits=[-5.5,5.5,-0.1,-1.3]  # gpd37 martina rot 0
#limits=[-5.5,5.5,-0.5,-1.7]  # gpd38 martina rot 0

#limits=[-0.5,0,4,-2]  # gpd35 stefano InSb 911_0000556
#limits=[1,4,2,-2]  # gpd35 stefano Si220   0000560
#limits=[-5,-3.5,4,-3]  # gpd35 stefano Si111 0000583
#limits=[-3,3,3,-3]  # gpd35 stefano Si400  0000604
#limits=[-0,4,0,-4]  # gpd35 stefano Se442  0000625
#limits=[0.5,1.5,4,-2]  # gpd35 stefano Ge111  0000625

#limits=[-2,3,2,-4]  # gpd35 stefano Ge111  0000625

#limits=[-1.5,1,4,-4]  # gpd38 simo InSb111 
...
base_cut= np.where((pha>0) & (pha<60000) & (x_abs<=x_max) & (x_abs>=x_min) &(y_abs<=y_max) & (y_abs>=y_min) &(livetime>15 )& (num_clu>0)   )[0]
