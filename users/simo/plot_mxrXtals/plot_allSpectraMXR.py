...
def plot_all(base_path,input_files,legend,norm_limits,x_lines,labels,labels_scale,calP1=1.5005121319323431 ,calP0=-19.137179036724653):
    ...
    for i in range(0,len(input_files)):
        ...
        for j in range(0, len(x_lines[i])):
           ...
           # plt.vlines(x=x_lines[i][j]/1000., ymin=0.001,ymax=labels_scale[i][j]*3,color='lightgray', linestyle='--')
            plt.vlines(x=x_lines[i][j]/1000., ymin=0.001,ymax=labels_scale[i][j]*3,color='gray', linestyle='--')
           
            ax.text(x_lines[i][j]/1000.,labels_scale[i][j]*4 ,labels[i][j], horizontalalignment=hor_al, verticalalignment='center', size=16, color='black')
    ...
    plt.yscale('log')
    plt.ylim(0.001,10)
    plt.xlim(200/1000.,13100/1000.)
    
    plt.legend()

    fig.savefig("allxtals_spectra.png")
    plt.show()
