...
print("counts2dCluAVE=",counts2dCluAVE)
  
counts2dCluAVE=counts2dCluAVE.T
...
im=ax1.imshow((counts2dCluAVE), interpolation='nearest', origin='lower',  extent=[xedges[0], xedges[-1], yedges[0], yedges[-1]])  
ax1.legend()
#ax1.colorbar()
...
if PLOT_MAP==True:
    ...
    im3=ax3.imshow(counts2dCluAVE, interpolation='nearest', origin='lower',  extent=[xedges[0], xedges[-1], yedges[0], yedges[-1]])  
    #plt.colorbar()
    ...
    cb3=fig3.colorbar(im3,cax=cax, orientation='vertical')
    ax3.set_title('meanE?')
...
### salvo mappa
np.savez("prova_unifMap.npz",corr= counts2dCluAVE, x_bins=xedges, y_bins=yedges)
...
# =======================
# correct spectrum given map
# ==============

print("cerco indici")
x_idx=np.searchsorted(xedges,x_all[myCut])-1
y_idx=np.searchsorted(yedges,y_all[myCut])-1

print("cerco correzioni")
corrw= counts2dCluAVE[y_idx,x_idx]

print("len(corrw)",len(corrw)," len(w_all[myCut])=", len(w_all[myCut]))

print("corrw=",corrw)

# plotto spettro corretto?????

countsCluCorr, binsECorr = np.histogram( w_all[myCut]/corrw  , bins = int(2*NBINS/2.), range = (-NBINS,NBINS) )
binsECorr=binsECorr*calP1+calP0
ax2.hist(binsECorr[:-1], bins = binsECorr, weights = countsCluCorr, histtype = 'step',label="energy CORRECTED")
ax2.set_xlabel('E[keV]')
#ax2.set_xlim([0,12])
ax2.set_yscale('log')
ax2.legend()
...
#for i in range(145678, len(energy_all[myCut])):
#      
#    x=np.searchsorted(xedges,x_all[myCut][i])-1
#    y=np.searchsorted(yedges,y_all[myCut][i])-1
#    corr= counts2dCluAVE[y][x]    
#    print(i, "x_all[i]=",x_all[myCut][i]," y_all=",y_all[myCut][i]," x=",x," y=",y," corr=",corr)
