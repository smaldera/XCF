...
shots_path ='/home/maldera/Desktop/eXTP/data/laserShots/h_6.3_p10000/'
bg_shots_path = '/home/maldera/Desktop/eXTP/data/laserShots/bkg/'
...
for image_file in tqdm(f, colour='green'):
   ...
   #istogramma 2d immagine raw dopo zero suppression:
    ...
    # X projection:
    countsX_i, binsX_i = np.histogram(zeroSupp_trasposta[0] ,  bins =xbins2d , range = (0,XBINS) ) 
    countsX_all=countsX_all+countsX_i
    #Y projection:
    countsY_i, binsY_i = np.histogram(zeroSupp_trasposta[1] ,  bins =ybins2d , range = (0,YBINS) ) 
...
#fig2=plt.figure(2)
h3.hist(binsX_i[:-1], bins = binsX_i, weights =   countsX_all, histtype = 'step',label="X projection")



fig3, h4 = plt.subplots()
h4.hist(binsY_i[:-1], bins = binsY_i, weights =   countsY_all, histtype = 'step',label="Y projection")
...
np.savez(shots_path+'X_dist_ZeroSupp'+pixMask_suffix+cluCut_suffix, counts = countsX_all, bins = binsX_i)
np.savez(shots_path+'Y_dist_ZeroSupp'+pixMask_suffix+cluCut_suffix, counts = countsY_all, bins = binsY_i)
