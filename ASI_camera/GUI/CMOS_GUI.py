...
if __name__ == "__main__":
    ...
    TCamera = [ #Terza tab per visualizzare la lista eventi
        ...
        [
            sg.Text('Number of parallel processes of analyze'),
    ...
    while True:
        ...
        if event == "_CAPTURE_AND_ANALYZE_PARALLEL_":
            try:
                CaptureAndAnalyze2(StoreDataIn, int(SampleSize),int(WBR),int(WBB),int(exposure),int(gain),bkg_folder_b, xyRebin2, sigma2, cluster2, NoClustering2, NoEvent2, Raw2, Eps2, int(num))
            except Exception as e:
                sg.popup(f"Cannot launch CaptureAndAnalyze: {e}")
