...
for n_meas in range (7,50 ):
   ...
   if  n_meas!=4:
      ...
      cmd='mkdir -p '+StoreDataIn
      os.system(cmd)
      OBJ = aotr2(StoreDataIn, sample_size, WB_R, WB_B, EXPO, GAIN, bkg_folder, xyRebin, sigma, cluster, ApplyClustering, SaveEventList,raw, Eps,num_jobs ,leng,save_every,camera_id=CAMERA_ID, LIVE_PLOTS=False,GUI=False,max_time=MAX_TIME)
      print("starting acq loop - AIR !!!!!!!!!!!!!!! :",StoreDataIn)  
      OBJ.CaptureAnalyze() 
      del OBJ
  ...
  # print("sleep 40 sec")
  # time.sleep(40)
  # print("tempo pausa=",time.time()-start)
  # print("")
   ...
   os.system(cmd)
   OBJ = aotr2(StoreDataIn, sample_size, WB_R, WB_B, EXPO, GAIN, bkg_folder, xyRebin, sigma, cluster, ApplyClustering, SaveEventList,raw, Eps,num_jobs ,leng,save_every,camera_id=CAMERA_ID, LIVE_PLOTS=False,GUI=False,max_time=MAX_TIME)
   print("starting acq loop - NORCADA !!!!!!!!!!!!!!!!!  :",StoreDataIn)  
   OBJ.CaptureAnalyze() 
   del OBJ
