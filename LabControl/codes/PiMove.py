...
class PiMikro:
    ...
    def Cobra(self, velocity=1, step = 0.5, amplitude=7, Pa=900, Px=107, Py=127, Nstep=None, verbose=False):
    ...
    def LowReach(self, Target, verbose=False):
        """Given a point it moves to it using Low axes"""
        if verbose:
            print(f'fastReach Target: {Target.GetX()} , {Target.GetY()}')

        self.xLow.MOV(1, Target.GetX())
        self.yLow.MOV(1, Target.GetY())
    ...
    def UniformQuake(self, width=0.5, Pup = Point2D(), verbose=False):
        # [width] = mm !!
        if verbose:
            print(f'EpiCenter : {Pup.GetX()} , {Pup.GetY()}')
        Pup.Shift(np.random.uniform(low=-width, high=width), np.random.uniform(low=-width, high=width))
        if verbose:
            print(f'Shifted in: {Pup.GetX()} , {Pup.GetY()}')
        self.fastReach(Pup)
        return 1

    def UnPolPosition(self, deg, move=True):
        Plow = Point2D()
        Pup = Point2D()
        if deg==-90:
            print(f'Degree: {deg}')
            Plow.Shift(23.5, 10.8)
            Pup.Shift(12.5, 12.5)
            #Plow.Shift(226.5, 8.5)
            #Pup.Shift(10.2, 16.9)
        elif deg == 0:
            print(f'Degree: {deg}')
            Plow.Shift(43, 3.5)
            ...
            print(f'! Degree {deg } not specified !')
            return 1, 1
        if move:
            self.xLow.VEL(1, 3)
            self.yLow.VEL(1, 3)
